...
export default {
  ...
  methods: {
    async loadOptions() {
      const response = await axios.get('http://localhost:8081/accounts/options',
          {
            headers: {
    ...
    async loadData() {
      const response = await axios.get('http://localhost:8081/accounts/' + (this.$route.params.id ? this.$route.params.id : this.account.id),
          {
            headers: {
              Authorization: this.user ? "Bearer " + this.user.token : null,
            },
