        ...
        <div class="px-4 py-6 sm:p-8">
          <div class="grid max-w-xl grid-cols-2 gap-x-6 gap-y-8 sm:grid-cols-6">
            <div class="sm:col-span-3">
              <label class="block text-sm font-medium leading-6 text-gray-900">task Title</label>
              <input v-model="task.title" :class="errors.title ? 'border border-red-300' : 'border-0'"
            ...
            <div class="sm:col-span-3">
              <label class="block text-sm font-medium leading-6 text-gray-900" for="last-name">task Date</label>

              <input v-model="task.task_date"
                     :class="errors.task_date ? 'border border-red-300' : 'border-0'"
                     type="date"
              ...
              <div v-if="errors.task_date" class="text-sm text-red-400">{{ errors.task_date }}</div>
            </div>
            <div class="sm:col-span-3">
              <label class="block text-sm font-medium leading-6 text-gray-900">Location</label>
              <input v-model="task.location" :class="errors.location ? 'border border-red-300' : 'border-0'"
                     class="block w-full p-2 rounded-md  py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            ...
            <div class="sm:col-span-3">
              ...
              <select v-model="task.contact_id" @change="task.account_id = null" :class="errors.contact_id ? 'border border-red-300' : 'border-0'"
                  class="bg-white  py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 block p-2 w-full rounded-md  placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            ...
            <div class="sm:col-span-3">
              ...
              <select v-model="task.account_id" @change="task.contact_id = null" :class="errors.account_id ? 'border border-red-300' : 'border-0'"
                  class="bg-white  py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 block p-2 w-full rounded-md  placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            ...
            <div class="sm:col-span-3">
              <label class="font-medium text-gray-900">Completed</label>
                <input v-model="task.completed" type="checkbox" class="h-4 w-4 flex rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
              </div>
            <div class="sm:col-span-3">
              <label class="block text-sm font-medium leading-6 text-gray-900">Notes</label>
              <textarea v-model="task.notes" :class="errors.notes ? 'border border-red-300' : 'border-0 '"
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      const response = await axios.get('http://localhost:8081/activities/' + (this.$route.params.id ? this.$route.params.id : this.task.id),
          {
            headers: {
              Authorization: this.user ? "Bearer " + this.user.token : null,
            },
